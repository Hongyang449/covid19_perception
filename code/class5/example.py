...
df0=pd.read_csv(path0 + 'comment_235_corrected.tsv',sep='\t')
index0= df0.loc[:,'label'] == 5
index1 = df0.loc[:,'label'] != 5
...
i = 4
the_text = text_pos[i].replace('|',' ').replace('nan','').replace('!',' !').replace(' .','.').replace('  ',' ').rstrip().lstrip()
...
#array([0.009, 0.013, 0.034, 0.043, 0.056, 0.081, 0.09 , 0.128, 0.154,
#       0.222, 0.231, 0.244, 0.35 , 0.38 , 0.47 , 0.598, 0.62 , 0.761,
#       0.91 , 0.974])
text_neg = df0.iloc[id_test[the_index0],1].tolist()
for i in range(len(the_index0)):
    print(i, pred_rank[the_index0][i], text_neg[i], pred_all[the_index0][i])
...
the_index = ~np.array([x=='' or x=='|' or x=='nan' or x=='. ' or x=='’' or x=='t ' or x=='m ' \
    or x=='vn' or x=="'" or x=='(' or x==')' or x==', ' or x=='.' or x=='ers' or x=='ve ' \
    or x=='gging ' or x==') ' for x in the_word])
...
the_index = ~np.array([x=='' or x=='|' or x=='nan' or x=='. ' or x=='’' or x=='t ' or x=='m ' \
    or x=='vn' or x=="'" or x=='(' or x==')' or x==', ' or x=='.' or x=='ers' or x=='ve ' \
    or x=='gging ' or x=='s ' or x=='cer' for x in the_word])
the_shap = shap_values.values[the_index0[i]][:,1][the_index]
# check length
len(the_text.split(' '))
the_shap.shape
...
i=19
the_text = text_neg[i].replace('|',' ').replace('nan','').replace('!',' !').replace('  ',' ').replace('/',' / ').rstrip().lstrip()
the_word = shap_values.data[the_index0[i]]
the_index = ~np.array([x=='' or x=='|' or x=='nan' or x=='. ' or x=='’' or x=='t ' or x=='m ' \
    or x=='vn' or x=="'" or x=='(' or x==')' or x==', ' or x=='.' or x=='ers' or x=='ve ' \
